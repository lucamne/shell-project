...
{
	char** argv = NULL;
	// parse all arguments from input string into a vector
	...
	int argc = 0;
	// used to traverse through argv
	char** argv_pos = argv;
	// can be passed to exec_command to redirect stdout for that program
	FILE* out_fd;
	FILE* err_fd;
	char* redirection_path = NULL;
	while (*argv_pos != NULL && pid != 0)
	{
		// check if redirection operator
		if (strcmp(*argv_pos, ">") == 0)
		{
			// set redirection iff there is one argument following '>' operator
			if (*(argv_pos) + 1 == NULL || strcmp(*(argv_pos + 1), "&") == 0)
			{
				print_error("Usage: > [file]");
			}
			else if (*(argv_pos + 2) == NULL || strcmp(*(argv_pos + 2), "&") == 0)
			{
				redirection_path = malloc(sizeof(char) * (strlen(*(argv_pos + 1)) + 1));
				strcpy(redirection_path, *(argv_pos + 1));
			}
			else 
			{
				print_error("Usage: > [file]");
		...
		else if (strcmp(*argv_pos, "&") == 0)
		{
			...
			{
				// size may differ from argc because it accounts for possibility of redirection path
				int argv_size = argc;
				// if rediretion path is assigned then open stream from path and assign to stdout
				if (redirection_path != NULL)
				{
					out_fd = freopen(redirection_path, "w+", stdout);
					err_fd = freopen(redirection_path, "w+", stderr);
					argv_size -= 2;
				}		

				// make copy of argv so that exec_command receives a pointer returned by malloc
				char** argv_cpy = malloc((argv_size + 1) * sizeof(char*));
				argv_cpy = memcpy(argv_cpy, argv_pos - argc, (argv_size + 1) * sizeof(char*));
				// NULL terminate array
				argv_cpy[argv_size] = NULL;
				// execute command
				exec_command(argv_size, argv_cpy, redirection_path);
				free(argv_cpy);
				if (redirection_path != NULL)
				{
					fclose(out_fd);
					fclose(err_fd);
					free(redirection_path);
